...
module "microservice" {
  ...
  namespace      = "stage" # Podemos usar el valor hardcodeado porque está garantizado que existe
  ...
  # Por ejemplo, si 'user-service' necesitara 2 réplicas en stage:
...
resource "kubernetes_deployment" "zipkin" {
  ...
  metadata {
    name      = "zipkin"
    namespace = "stage" # Usamos el valor directamente
    labels    = { app = "zipkin" }
  }
